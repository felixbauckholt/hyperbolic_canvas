...
def compute_stuff(poly, n, f):
    ...
    while 1:
        ...
        try:
            ...
            i_sqrt_zeta = rotate(Integer(1), Integer(1)/Integer(2*poly), r)
            print "p=", p, "l=", i_l, "zeta=", i_zeta
            f.write("    {%f, %d, %d, %d, %s, %s, %d},\n" % (l, poly, p, Integer(i_l), get_complex(i_zeta, r),
                get_complex(i_sqrt_zeta, r), n))
            break
...
f.write("    -- {l (real), sides, p, i_l (scalar mod p), i_zeta (complex mod p), i_sqrt_zeta (complex mod p), atvertex}\n")
